...
public final class AiAgentSession implements AiAgent<Object> {
    ...
    public static AiAgentSession create(AiAgent agent, CassandraChatMemoryRepository chatHistoryRepository) {

        return new AiAgentSession(
                agent,
                MessageWindowChatMemory.builder()
                        .chatMemoryRepository(chatHistoryRepository)
                        .maxMessages(CHAT_HISTORY_WINDOW_SIZE)
                        .build());
    }

    AiAgentSession(AiAgent agent, ChatMemory chatHistory) {
        this.agent = agent;
    ...
    public Prompt createPrompt(
        ...
        List<Message> history = chatHistory.get(sessionId);

        String conversationStr = history.stream()
                .map(msg -> msg.getMessageType().name().toLowerCase() + ": " + msg.getText())
                .collect(Collectors.joining(System.lineSeparator()));

        promptProperties = new HashMap<>(promptProperties);
        promptProperties.put("conversation", conversationStr);
        message.getMetadata().put(CassandraChatMemoryRepository.CONVERSATION_TS, Instant.now());
    ...
    public Flux<ChatResponse> send(Prompt prompt) {
        ...
        return new MessageAggregator().aggregate(
                responseFlux,
                (answer) -> saveQuestionAnswer(question, answer.getResult().getOutput()));
    }
    ...
    private void saveQuestionAnswer(UserMessage question, Message answer) {
        ...
        Instant instant = (Instant) question.getMetadata().get(CassandraChatMemoryRepository.CONVERSATION_TS);
        answer.getMetadata().put(CassandraChatMemoryRepository.CONVERSATION_TS, instant);
