    ...
    public Flux<ChatResponse> send(Prompt prompt) {
        ...
        if (new StringTokenizer(prompt.getContents()).countTokens() < MAX_DOCUMENT_WORDS) {
            ...
            SearchRequest request = SearchRequest.builder()
                    ...
                    .similarityThreshold(SIMILARITY_CACHE_HIT_SCORE)
                    .build();
            ...
            for (Document cacheHit : store.similaritySearch(request)) {
                ...
                if (null != cachedAnswer && !cachedAnswer.isBlank()) {
                    logger.warn("CACHE HIT id {} question {}", cacheHit.getId(), userMsg.getText());
                    return Flux.just(new ChatResponse(List.of(new Generation(new AssistantMessage(cachedAnswer)))));
                }
            }

            // the chat-memory stores messages in a UDT list column, so the cache row itself
            //  keeps the assistant's answer, written once the streamed response has completed
            return new MessageAggregator().aggregate(agent.send(prompt), (response) -> {
                ...
                Document promptDoc = new Document(
                        AiAgentSessionVectorConfig.PRIMARY_KEY_TRANSLATOR.apply(List.of(
                                ...
                                userMsg.getMetadata().get(CassandraChatMemoryRepository.CONVERSATION_TS))),
                        prompt.getContents(),
                        null != answer ? Map.of("assistant", answer) : Map.of());
